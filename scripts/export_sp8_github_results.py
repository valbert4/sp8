...
def read_simple_nonabelian_summary(out_dir: Path) -> list[dict[str, object]]:
    path = out_dir / "simple_nonabelian_representatives.tsv"
    if not path.exists():
        return []
    ...
    with path.open(newline="") as f:
        for row in csv.DictReader(f, delimiter="\t"):
            ...
            grouped[key]["classes"] = int(grouped[key]["classes"]) + 1
            if row.get("actual_subgroups"):
                grouped[key]["actual_subgroups"] = int(grouped[key]["actual_subgroups"]) + int(row["actual_subgroups"])
            grouped[key]["rep_ids"].append(int(row["rep_id"]))  # type: ignore[union-attr]
    return sorted(grouped.values(), key=lambda item: (int(item["order"]), str(item["structure"])))


def markdown_simple_nonabelian_section(out_dir: Path) -> str:
    rows = read_simple_nonabelian_summary(out_dir)
    if not rows:
        return (
            "## Nonabelian Simple Subgroups\n\n"
            "The nonabelian simple subgroup table is generated by "
            "`scripts/find_simple_nonabelian_subgroups.py`."
        )
    simple_classes = sum(int(row["classes"]) for row in rows)
    actual_subgroups = sum(int(row["actual_subgroups"]) for row in rows)
    lines = [
        "## Nonabelian Simple Subgroups",
        "",
        f"A GAP scan of the {fmt_int(SP8_NONSOLVABLE_CLASSES)} nonsolvable representatives found "
        f"{fmt_int(simple_classes)} nonabelian simple `Sp(8,2)`-conjugacy classes, spanning "
        f"{fmt_int(len(rows))} abstract isomorphism types. The table gives the number of "
        "`Sp(8,2)`-classes of each type and the total number of actual subgroups in those "
        f"classes, computed from the normalizer pass; together these classes account for "
        f"{fmt_int(actual_subgroups)} actual subgroups. The `O(9,2)` / `Sp(8,2)` row is "
        "the ambient group itself.",
        "",
        "| Simple group | Alias | Order | `Sp(8,2)`-classes | Actual subgroups | Representative ids |",
        "| --- | --- | ---: | ---: | ---: | --- |",
    ]
    for row in rows:
        rep_ids = ", ".join(str(rep_id) for rep_id in row["rep_ids"])  # type: ignore[union-attr]
        lines.append(
            "| {structure} | {alias} | {order} | {classes} | {actual} | `{rep_ids}` |".format(
                structure=row["structure"],
                alias=row["alias"],
                order=fmt_int(int(row["order"])),
                classes=fmt_int(int(row["classes"])),
                actual=fmt_int(int(row["actual_subgroups"])),
                rep_ids=rep_ids,
            )
        )
    return "\n".join(lines)
...
{markdown_simple_nonabelian_section(out_dir)}
